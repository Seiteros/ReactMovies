  ...
  try {
    ...
    if (data.year > todayDate.getFullYear() || data.year < 1000) {
      ...
      const insertedRow = await client.query(
        "INSERT INTO movies (title, director, genre, year, description, image_url, rating_count, rating_result) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *",
        [data.title, data.director, data.genre, data.year, data.description, data.image_url, countPerson, countPerson * score]
      );
...
app.patch("/movie/:id/rate", async (req, res) => {
  ...
  if (!movie.rows[0]) {
    ...
    if (score < 1 || score > 5) {
      ...
      try {
        await client.query("UPDATE movies SET rating_count=$1, rating_result=$2 WHERE id = $3", [
          movieFromDb.rating_count + 1,
          movieFromDb.rating_result + score,
          id,
        ]);
