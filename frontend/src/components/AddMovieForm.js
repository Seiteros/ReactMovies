      ...
      validate={(values) => {
        ...
        function validateImage_url(image_url) {
          ...
          if (!testUrl(image_url)) {
            errors.image_url = "Nieprawidłowy url obrazka";
          }
        }
    ...
    >
      {({ isSubmitting }) => (
        <Form>
          ...
          <Field name="description" type="text" as="textarea" className="form-control bg-dark text-light" />
          ...
          <button type="submit" className="btn btn-success m-1" disabled={isSubmitting}>
            Wyślij
          </button>
        </Form>
      )}
